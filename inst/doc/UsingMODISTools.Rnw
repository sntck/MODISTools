...
Alternatively, we may want transects of MODIS data. This is easily done by specifying start and end points ofr transects and calling \code{MODISTransects}. Our data here does not have coordinates that specify transect end points yet, so we need to calculate them using \code{EndCoordinates}.
<<eval=FALSE>>=
...
<<eval=FALSE>>=
TileExample <- read.csv(list.files(pattern = "Data"))
