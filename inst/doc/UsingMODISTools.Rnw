...
<<>>=
dates <- GetDates(Product = "MOD13Q1", Lat = modis.subset$lat[1], Long = modis.subset$long[1])
min(dates)
max(dates)
@
...
head(ExtractTile(Data = TileExample, Rows = c(9,2), Cols = c(9,2), Grid = FALSE), n = 2)
@
...
land.summary <- read.csv(paste("./LandCover/",
                               list.files(path = "./LandCover/",
                                          pattern = "MODIS Land Cover Summary"),
                               sep = ""))
head(land.summary)
@
