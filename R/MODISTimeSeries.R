...
function(Dir, Band, Simplify = FALSE)
{
  ...
  file.ids <- sapply(file.path(Dir, file.set), function(x)
                     any(grepl(Band, read.csv(x, header = FALSE, as.is = TRUE)[ ,modisMethods$whereIsBandName])))
  ...
  for(i in 1:length(file.set)){
    ...
    data.collector[[i]] <- as.matrix(data.file[ ,(modisMethods$numberOfMetadataCols+1):ncol(data.file)])

    nrow.recorder[i] <- nrow(as.matrix(data.file[ ,(modisMethods$numberOfMetadataCols+1):ncol(data.file)]))
    ncol.recorder[i] <- ncol(as.matrix(data.file[ ,(modisMethods$numberOfMetadataCols+1):ncol(data.file)]))
