...
function(LoadData, FileSep = NULL, Product, Bands, Size, SaveDir = ".", StartDate = FALSE, TimeSeriesLength = 0)
{
  ...
  # If the Product input does not match any product codes in the list output from modisMethods$getProducts(), stop with error.
  if(!any(Product == modisMethods$getProducts())) stop("Product entered does not match any available products.")

  # If the Bands input does not match with the Product input, stop with error.
  band.test <- lapply(Bands, function(x) !any(x == modisMethods$getBands(Product)))
  if(any(band.test == TRUE)) stop("A Band does not match Product.")
  ...
  cell.size.dates <- modisMethods$getDates(Lat = LoadData$start.lat[1], Long = LoadData$start.long[1], Product = Product)[1:2]
  cell.size <- as.numeric(unname(
    modisMethods$getSubset(Lat = LoadData$start.lat[1], Long = LoadData$start.long[1], Product = Product, Band = Bands[1],
  ...
  for(i in 1:nrow(t.dat)){
    ...
    lat.metres <- delta.lat * modisMethods$metresInOneDegree
    ...
    long.metres <- delta.long * (modisMethods$metresInOneDegree * cos(lat.rad))
    ...
    date.for.xy <- modisMethods$getDates(t.subset$lat[1], t.subset$long[1], Product)[1]

    for(n in 1:nrow(t.subset)){
      t.point <- modisMethods$getSubset(t.subset$lat[n], t.subset$long[n], Product,
