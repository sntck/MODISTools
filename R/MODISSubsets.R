      ...
      for(n in 1:length(Bands)) {               
        ...
        if(ncol(date.list) > 1) {               
          ...
          for(x in 1:(ncol(date.list)-1)) {     
            ...
            result[[n]]<- try(GetSubset(lat.long[i,2], lat.long[i,3], Product, Bands[n], 
            ...
            subsets<- as.vector(c(subsets, result[[n]]$subset[[1]]))
          }
        ...
        result[[n]]<- try(GetSubset(lat.long[i,2], lat.long[i,3], Product, Bands[n], date.list[1,ncol(date.list)],
        ...
        if(regexpr("Server is busy handling other requests in queue. Please try your subset order later.We apologize 
              for the inconvenience", result[[n]]$subset[[1]][1]) != -1) {
