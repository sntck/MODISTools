...
function(LoadDat, LoadMethod='object' | 'ext.file', FileSep=NULL, Product, Bands, Size=c(), SaveDir="./", StartDate=FALSE, TimeSeriesLength=2, DateFormat='year' | 'posixt', Transect=FALSE)
{
    ...
    ifelse(Start == TRUE, lat.long<- unique(cbind(lat=dat$lat[!is.na(dat$lat)],long=dat$long[!is.na(dat$lat)],end.date=dat$end.date[!is.na(dat$lat)],start.date=dat$start.date[!is.na(dat$lat)])), lat.long<- unique(cbind(lat=dat$lat[!is.na(dat$lat)],long=dat$long[!is.na(dat$lat)],end.date=dat$end.date[!is.na(dat$lat)])))
    ...
    if(DateFormat == 'year') {
        options(warn=-1); ifelse(Start == FALSE, start.date<- strptime(paste(lat.long[,4]-TimeSeriesLength,'-01-01',sep=''),'%Y-%m-%d'), start.date<- strptime(paste(lat.long[,5],'-01-01',sep=''),'%Y-%m-%d')); options(warn=0)
    ...
    if(DateFormat == 'posixt') {
        options(warn=-1); ifelse(Start == FALSE, start.date<- strptime(paste(lat.long[,4]-TimeSeriesLength,'-01-01',sep=''),'%Y-%m-%d'), start.date<- strptime(lat.long[,5],'%Y-%m-%d')); options(warn=0)
    ...
    if(Transect == FALSE){ write.table(lat.long, file=paste(SaveDir,'Subset Download ',Sys.Date(),'.csv',sep=''), col.names=TRUE, row.names=FALSE, sep=',') }
    if(Transect == TRUE){
