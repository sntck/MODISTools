         ...
         Yield = FALSE, Interpolate = FALSE, InterpolateN = NULL, DiagnosticPlot = FALSE)
{
  ...
  product.bands <- try(modisMethods$getBands(Product), silent = TRUE)
  ...
  size.test <- sapply(file.path(Dir, file.list), function(x) ncol(read.csv(x)[1, ]) - modisMethods$numberOfMetadataCols)
  ...
  for(counter in 1:length(file.list)){
    ...
    names(ds) <- c("nrow", "ncol", "xll", "yll", "pixelsize", "row.id", "product.code", "MODIS.acq.date",
                   "where", "MODIS.proc.date", 1:(ncol(ds) - modisMethods$numberOfMetadataCols))
    ...
    ds <- cbind(ds[,1:modisMethods$numberOfMetadataCols], date, ds[,(modisMethods$numberOfMetadataCols+1):ncol(ds)])
