...
function(Dir = ".", Band)
{
  ...
  if(length(file.list) == 0)
    stop("Found no MODIS Land Cover ASCII files in Dir.")
  if(!any(modisMethods$getBands("MCD12Q1") == Band))
    stop("LandCover is for land cover data. Band specified is not for this product.")

  lc.type.set <- modisMethods$landCoverClasses[[which(names(modisMethods$landCoverClasses) == Band)]]
  ...
  for(i in 1:length(file.list)){
    ...
    names(lc.subset) <- c("nrow", "ncol", "xll", "yll", "pixelsize", "row.id", "land.product.code",
                          "MODIS.acq.date", "where", "MODIS.proc.date", 1:(ncol(lc.subset) - modisMethods$numberOfMetadataCols))
    ...
    lc.tiles <- as.matrix(lc.subset[which.are.band,(modisMethods$numberOfMetadataCols+1):ncol(lc.subset)],
                          nrow = length(which.are.band), ncol = length((modisMethods$numberOfMetadataCols+1):ncol(lc.subset)))
    ...
    for(x in 1:nrow(lc.tiles)){
      ...
      no.fill[x] <- paste(round((sum(lc.subset[x,(modisMethods$numberOfMetadataCols+1):ncol(lc.subset)] == NoDataFill) / length(lc.tiles[x, ])) * 100, 2),
                          "% (", sum(lc.subset[x,(modisMethods$numberOfMetadataCols+1):ncol(lc.subset)] == NoDataFill), "/", length(lc.tiles[x, ]), ")",
                          sep = "")
