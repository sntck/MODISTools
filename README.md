...
Recent stable releases of this package have been checked and built on Windows, Mac, and Linux, and last checked on R 3.0.3 on 26/03/2014. `MODISTools` is written by [Sean Tuck](https://github.com/seantuck12) and [Helen Phillips](https://github.com/helenphillips). This package can be used under the terms of the GNU GPLv3 license; feel free to use and modify as you wish, but please cite my work where appropriate.

Changes in recent updates:
* New function, `MODISTimeSeries`, to produce one matrix for all timeseries data downloaded, with a row for each date in the timeseries and a column for each unique timeseries (i.e., each ASCII file downloaded).
...
Changes coming soon:
* Improved handling of POSIX dates.
* More product-specific time-series summaries in MODISSummaries function.
